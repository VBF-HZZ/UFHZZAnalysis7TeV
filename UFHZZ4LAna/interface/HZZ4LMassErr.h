...
class HZZ4LMassErr
{
	...
	public:
		...
		double calc2e2muErr( vector< pat::Electron > electrons, vector< pat::Muon > muons, vector< pat::PFParticle > fsrPhotons, bool corr, bool isData);
		...
		//ForZ
		double calc2eErr(  vector< pat::Electron > electrons, bool corr, bool isData);
		double calc2muErr( vector< pat::Muon > muons, bool corr, bool isData);
...
HZZ4LMassErr::HZZ4LMassErr()
{
	...
	fmu = boost::shared_ptr<TFile>( new TFile("UFHZZAnalysis7TeV/UFHZZ4LAna/hists/ebeOverallCorrections.LegacyPaper.42x.root") ); 
	fel = boost::shared_ptr<TFile>( new TFile("UFHZZAnalysis7TeV/UFHZZ4LAna/hists/ebeOverallCorrections.LegacyPaper.42x.root") );

	muon_corr_data = boost::shared_ptr<TH2F>(  (static_cast<TH2F*>(fmu->Get( "mu_reco42x" )->Clone() )) );
        muon_corr_mc = boost::shared_ptr<TH2F>(  (static_cast<TH2F*>(fmu->Get( "mu_mc42x" )->Clone() )) );
	electron_corr_data = boost::shared_ptr<TH2F>(  (static_cast<TH2F*>(fel->Get( "el_reco42x" )->Clone() )) ); 
        electron_corr_mc = boost::shared_ptr<TH2F>(  (static_cast<TH2F*>(fel->Get( "el_mc42x" )->Clone() )) );
}
...
void HZZ4LMassErr::fillP3Covariance(const reco::Muon &c, TMatrixDSym &bigCov, int offset) const {
	fillP3Covariance(c, *c.innerTrack(), bigCov, offset);
}
...
double HZZ4LMassErr::calc4muErr( vector< pat::Muon > muons, vector< pat::PFParticle > fsrPhotons, bool corr, bool isData ){
	...
	for(unsigned int i=0; i< muons.size(); i++){
		...
		vpterr.push_back(mu->innerTrack()->ptError() * scaleFactor_mu );
...
double HZZ4LMassErr::calc2e2muErr( vector< pat::Electron > electrons, vector< pat::Muon > muons, vector< pat::PFParticle > fsrPhotons, bool corr, bool isData ){
	...
	for(unsigned int i=0; i< muons.size(); i++){
		...
		vpterr.push_back(mu->innerTrack()->ptError() * scaleFactormu );
		vtlv.push_back(tlv);
	}
...
double HZZ4LMassErr::calc2muErr( vector< pat::Muon > muons , bool corr, bool isData){
	//	if(muons.size() != 4) printf("[calc4muErr] Warning: number of muons is not 4, but %d \n", (int)(muons.size()));
	vector<TLorentzVector> vtlv; vtlv.clear();
	vector<double> vpterr; vpterr.clear();      
...
double HZZ4LMassErr::calc2eErr( vector< pat::Electron > electrons, bool corr, bool isData ){
	//	if(electrons.size() != 4) printf("[calc4eErr] Warning: number of electrons is not 4, but %d \n", (int)electrons.size());
	vector<TLorentzVector> vtlv; vtlv.clear();
	vector<double> vpterr; vpterr.clear();
